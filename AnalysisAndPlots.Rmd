...
pairwise_interact<-function(data, resp){
  data %>% 
    mutate(interaction = paste(block, treatment))
  
  pairwise.t.test(data[[resp]], data[["interaction"]], p.adj="bonf")
}

#examples
# pairwise_interact(merlot2019, "berry.weight")
```
...
# 
# for (i in length(sub_response)) {
#   pairwise_interact(merlot2019, sub_response[i])
# }
```

```{r}
# helpers
se.single <- function(dat, response, colnum) {
  dat_response = dat[,colnum]
  dat = dat[,c(2,3,4)]
  
  # grand mean
  eta = mean(dat_response)
  
  # treatment means
  lvls_tr = levels(dat$treatment)
  treatment_means = matrix(nrow = length(lvls_tr))
  for (i in 1:length(lvls_tr)) {
    treatment_means[i,1] = mean(dat_response[which(dat$treatment == lvls_tr[i])])
  }
  
  # block means
  lvls_b = levels(dat$block)
  block_means = matrix(nrow = length(lvls_b))
  for (i in 1:length(lvls_b)) {
    block_means[i,1] = mean(dat_response[which(dat$block == i)])
  }
  
  # block_treatment means
  comb_means = matrix(nrow = length(lvls_tr), ncol = length(lvls_b))
  for (i in 1:length(lvls_tr)) {
    for (j in 1:length(lvls_b)) {
      comb_means[i,j] = mean(dat_response[which(dat$block == j & dat$treatment == lvls_tr[i])]) 
    }
  }
  ...
  for (i in 1:length(lvls_tr)) {
    for (j in 1:length(lvls_b)) {
      comb_obs = dat_response[which(dat$block == j & dat$treatment == lvls_tr[i])]
      SS_error = SS_error + sum((comb_obs - comb_means[i,j])^2)
    }
  }
  
  n = length(dat_response[which(dat$block == 1 & dat$treatment == lvls_tr[1])])
  I = length(lvls_tr)
  J = length(lvls_b)
  
  MSE = SS_error / ((n-1)*I*J)
  
  se_int = sqrt(MSE*(I-1)*(J-1)/(n*I*J))
  se_I = sqrt(MSE*(I-1)/(n*I*J))
  se_J = sqrt(MSE*(J-1)/(n*I*J))
  
  return(list(se_I, se_J, se_int))
}

effect.single <- function(dat, response, colnum) {
  dat_response = dat[,colnum]
  dat = dat[,c(2,3,4)]
  ...
  tr_effects = treatment_means - eta
  b_effects = block_means - eta
  int_effects = comb_means
  for (i in 1:length(lvls_tr)) {
    for (j in 1:length(lvls_b)) {
      int_effects[i,j] = comb_means[i,j] - treatment_means[i] - block_means[j] + eta
    }
  }

  return(list(tr_effects, b_effects, int_effects))
}

effect <- function(dat, response, colnum) {
  comb_effects = effect.single(dat, response, colnum)
  return(comb_effects)
}

se <- function(dat, response, colnum) {
  comb_se = se.single(dat, response, colnum)
  return(comb_se)
}
```
...
# 
# # get colnums
# cnames = colnames(merlot2019)
# colnums = rep(0,length(sub_response))
# for (i in 1:length(sub_response)) {
#   colnums[i] = which(cnames == sub_response[i])
# }
# 
# for (i in 1:length(sub_response)) {
#   print(sub_response[i])
#   eff = effect(merlot2019, sub_response[i], colnums[i])[[3]]
#   dimnames(eff)[[2]] <- 1:6
#   dimnames(eff)[[1]] <- c("treatment", "control")
#   print(eff)
# }
# 
# for (i in 1:length(sub_response)) {
#   print(sub_response[i])
#   ses = se(merlot2019, sub_response[i], colnums[i])[[3]]
#   print(ses)
#   
#   c = qr()
# }
