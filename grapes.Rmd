...
for (i in 1:length(resp.mer19)) {
  ...
  bart.p <- bartlett.test(eval(parse(text=resp.mer19[i])) ~ paste(treatment, block), data=merlot2019)$p.value
  lev.p  <- with(merlot2019, levene.test(eval(parse(text=resp.mer19[i])), paste(treatment, block))$p.value)
...
for (i in 1:length(resp.mer20)) {
  ...
  # the lapply function simply extracts the p-value from each outputted run of the MICE data
  bart.p <- bartlett.test(eval(parse(text=resp.mer20[i])) ~ paste(treatment, block), data=complete(merlot2020mice, 1))$p.value
  lev.p  <- with(complete(merlot2020mice, 1), levene.test(eval(parse(text=resp.mer20[i])), paste(treatment, block))$p.value)
...
# Boxplots of the first imputation dataset for the variables with potentially
# unequal variances
par(mfrow=c(sum(rowSums(uneq.var.mer20)>0),1))
lapply(rownames(uneq.var.mer20)[rowSums(uneq.var.mer20)>0], function(x){
  with(merlot2019,
       boxplot(cluster.weight ~ paste0(block, treatment))
       )[[1]]})
```
...
icen.mer19 <- with(merlot2019, {
  ...
  icen.mer19$verais.start[right.ver] <- ver.t1
...
```{r define.censor.plot.data}
# Let's make a plot to better visualize the censored data
censor.plot.mer20 <- icen.mer20
censor.plot.mer20$verais.point <- ifelse(censor.plot.mer20$verais.indicator %in% 
                                           c("left-censored", "observed"),
                                         censor.plot.mer20$verais.end,
                                         ifelse(censor.plot.mer20$verais.indicator=="right-censored",
                                         censor.plot.mer20$verais.start, NA))
censor.plot.mer20$verais.indicator <- factor(censor.plot.mer20$verais.indicator,
                                                levels=c("observed", "right-censored", "left-censored"))
censor.plot.mer20$order.verais <- order(censor.plot.mer20$verais.point)
censor.plot.mer20$bloom.point <- ifelse(censor.plot.mer20$bloom.indicator %in% 
                                           c("left-censored", "observed"),
                                         censor.plot.mer20$bloom.end,
                                         ifelse(censor.plot.mer20$bloom.indicator=="right-censored",
                                         censor.plot.mer20$bloom.start, NA))
censor.plot.mer20$bloom.indicator <- factor(censor.plot.mer20$bloom.indicator,
                                                levels=c("observed", "right-censored", "left-censored"))
censor.plot.mer20$order.bloom <- order(censor.plot.mer20$bloom.point)

```

```{r ver.2020}
ver20.cplot <- ggplot(censor.plot.mer20, aes(x=verais.point, y=order.verais, color=trt, shape=verais.indicator)) +
  geom_point() +
  # Style the graph
  xlim(c(10, 40)) + labs(title="Merlot 2020 Veraison Data") +
  xlab("Day") + ylab("Sample") + theme_bw() +
  
  scale_shape_manual(values=c(19, 0, 1), name="Start/end of intervals") +
  
  # Draw dotted lines for right-censored data
  geom_linerange(xmax=40, aes(xmin = verais.point, 
                              linetype = as.character(as.numeric(verais.indicator))),
                 size=1) +
  # Draw dotted lines for left-censored data
  geom_linerange(xmin = 10, aes(xmax = verais.point, 
                                linetype = as.character(as.numeric(verais.indicator)+3)),
                 size=1) +  
  # Draw solid lines for non-censored data
  geom_linerange(xmin = 15, aes(xmax = verais.point, 
                                linetype = as.character(as.numeric(verais.indicator)+6)),
                 size=1) + 
  scale_linetype_manual(values=c("1"="blank", "2"="dotted", "3"="blank",
                                 "4"="blank", "5"="blank", "6"="dotted",
                                 "7"="solid", "8"="solid", "9"="blank"),
                        guide=FALSE) +
  
  # Add in the T1 and T3 times
  geom_vline(xintercept=ver.t1, linetype="dashed", color="black") +
  geom_vline(xintercept=ver.t3, linetype="dashed", color="black") +
...
bloom20.cplot <- ggplot(censor.plot.mer20, aes(x=bloom.point, y=order.bloom, color=trt, shape=bloom.indicator)) +
  geom_point() +
  # Style the graph
  xlim(c(10, 40)) + labs(title="Merlot 2020 Bloom Data") +
  xlab("Day") + ylab("Sample") + theme_bw() +
  
  scale_shape_manual(values=c(19, 0, 1), name="Start/end of intervals") +
  
  # Draw dotted lines for right-censored data
  geom_linerange(xmax=40, aes(xmin = bloom.point, 
                              linetype = as.character(as.numeric(bloom.indicator))),
                 size=1) +
  # Draw dotted lines for left-censored data
  geom_linerange(xmin = 10, aes(xmax = bloom.point, 
                                linetype = as.character(as.numeric(bloom.indicator)+3)),
                 size=1) +  
  # Draw solid lines for non-censored data
  geom_linerange(xmin = 15, aes(xmax = bloom.point, 
                                linetype = as.character(as.numeric(bloom.indicator)+6)),
                 size=1) + 
  scale_linetype_manual(values=c("1"="blank", "2"="dotted", "3"="blank",
                                 "4"="blank", "5"="blank", "6"="dotted",
                                 "7"="solid", "8"="solid", "9"="blank"),
  ...
  # Add in the T1 and T3 times
  geom_vline(xintercept=bloom.t1, linetype="dashed", color="black") +
  geom_vline(xintercept=bloom.t3, linetype="dashed", color="black") +
  geom_vline(xintercept=15, linetype="dashed", color="black")
...
censor.plot.mer19 <- icen.mer19
censor.plot.mer19$verais.point <- ifelse(censor.plot.mer19$verais.indicator %in% 
                                           c("left-censored", "observed"),
                                         censor.plot.mer19$verais.end,
                                         ifelse(censor.plot.mer19$verais.indicator=="right-censored",
                                         censor.plot.mer19$verais.start, NA))
censor.plot.mer19$verais.indicator <- factor(censor.plot.mer19$verais.indicator,
                                                levels=c("observed", "right-censored", "left-censored"))
censor.plot.mer19$order.verais <- order(censor.plot.mer19$verais.point)


ver19.cplot <- ggplot(censor.plot.mer19, aes(x=verais.point, y=order.verais, color=trt, shape=verais.indicator)) +
  geom_point() +
  # Style the graph
  xlim(c(5, 40)) + labs(title="Merlot 2019 Veraison Data") +
  xlab("Day") + ylab("Sample") + theme_bw() +
  ...
  geom_linerange(xmax=40, aes(xmin = verais.point, 
                 ...
                 size=1) +
  # Draw dotted lines for left-censored data
  geom_linerange(xmin = 5, aes(xmax = verais.point, 
                                linetype = as.character(as.numeric(verais.indicator)+3)),
                 size=1) +  
  # Draw solid lines for non-censored data
  geom_linerange(xmin = 10, aes(xmax = verais.point, 
...
```{r combine.censor.plots}
library(gridExtra)
grid.arrange(ver19.cplot, ver20.cplot, bloom20.cplot, nrow=2, ncol=2)
