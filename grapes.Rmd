...
### chardonnay 2020
```{r}
char2019.resp <- select(chardonnay2020, (-1:-5))
cor(char2019.resp)
```
...
cor<-cor(merlot2019.resp)
corrplot(cor,type = "upper", order = "hclust", tl.col = "black", tl.srt = 45, addCoef.col="white", number.cex=0.7)
cor
```

We found out there are a few response variables are correlated: 
berry.TA and berry.PH: -0.5513 (ignorable)
cluster number and yield: 0.6132 (ignorable)
pruning weight and Ravaz.index: -0.6060 (ignorable)
yield and cluster weight: 0.8163
yield and berries.cluster: 0.7482
cluster weight and berries cluster: 0.9518

As cluster weight = yield/cluster number, and berries.cluster = cluster weight/berry weight. Hence, yield, cluster weight and berries cluster are associated. It is reasonable to include only one variable in analysis: cluster weight?

### merlot2020
- merlot2020 using all responses including the filled-in zeros
```{r}
merlot2020mice.resp <- select(complete(merlot2020mice, 1),(-1:-5))
cor<-cor(merlot2020mice.resp)
corrplot(cor,type = "upper", order = "hclust", tl.col = "black", tl.srt = 45, addCoef.col="white", number.cex=0.7)
cor
```
cluster number and yield: 0.9264
cluster number and berry.Brix -0.7146
veraision and berry.TA 0.7138
berry.TA and berry.Brix 0.76
berry.TA and berry.PH 0.87
berry weight and cluster weight 0.74
cluster weight and berries.cluster 0.95
berries.cluster and berry.Brix 0.74
berry.Brix and berry PH 0.75

again, berries.cluster = cluster weight/berry weight. keep just one variable: cluster weight?

- merlot2020 using all responses without the filled-in zeros
```{r}
merlot2020nozero.resp <- select(complete(merlot2020miceNoZero, 1),(-1:-5))
cor<-cor(merlot2020nozero.resp)
corrplot(cor,type = "upper", order = "hclust", tl.col = "black", tl.srt = 45, addCoef.col="white", number.cex=0.7)
cor
```
cluster weight and berries.cluster 0.95
cluster number and yield 0.93

The berries.cluster( = cluster weight/berry weight) and cluster weight are positively associated in merlot datasets for both years. We may need to keep only one of these two variables. In 2020, cluster number is highly correlated with yield, while in 2019, yield is correlated with cluster weight. It makes sense to keep yield only for both years.
