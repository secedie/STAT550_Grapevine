...
var_explained = (res$sdev^2/(sum(res$sdev^2)))[1] # 0.803176
...
```
### Merlot individual factors

```{r}

library(ARTool)

art.model.mer20 <- vector("list", length(resp.mer20))
anova.mer20 <- vector("list", length(resp.mer20))
names(anova.mer20) <- resp.mer20
names(art.model.mer20) <- resp.mer20

for (i in 1:length(resp.mer20)) {
  art.model.mer20[[i]] = art(eval(parse(text=resp.mer20[i])) ~ treatment + block + treatment:block,
                             data=complete(merlot2020mice, 1))
  anova.mer20[[i]] = anova(art.model.mer20[[i]], type = 1)
}

anova.mer20 <- lapply(anova.mer20, function(x){as.data.frame(x[,-1])})
anova.mer20 <- lapply(1:length(anova.mer20), 
                      function(x){cbind(anova.mer20[[x]], 
                                        data.frame("Var"=rep(names(anova.mer20)[x],3),
                                                   "Covar"=rownames(anova.mer20[[x]])))})

anova.mer20 <- do.call(rbind.data.frame, anova.mer20)

library(maditr)
dcast(anova.mer20, Var~Covar, value.var="Pr(>F)")
